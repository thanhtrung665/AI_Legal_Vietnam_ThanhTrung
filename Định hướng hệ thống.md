...
# CHI TIẾT CÁC BƯỚC TRIỂN KHAI (PIPELINE)

**Bước 1:** Tiền xử lý & Nạp dữ liệu (Data Ingestion & Parsing)

**Đối với file .docx:** Sử dụng thư viện Pandoc hoặc Mammoth. Chúng chuyển đổi file Word sang Markdown giữ nguyên cấu trúc heading và list rất mượt mà.

**Đối với file .pdf (Khó hơn rất nhiều):** Đừng dùng PyMuPDF cơ bản nữa. Hãy dùng các công cụ chuyên dụng cho RAG như:

- Docling (Open-source từ IBM): Đang là top 1 hiện nay về việc bóc tách PDF phức tạp (có bảng biểu, format hai cột) sang Markdown.

- Marker (từ Vik Paruchuri): Một thư viện mã nguồn mở dùng mô hình deep learning nhỏ để nhận diện cấu trúc PDF và xuất ra Markdown cực kỳ xuất sắc.

- LlamaParse: Công cụ chuyên dụng của hệ sinh thái LlamaIndex (có API miễn phí giới hạn), sinh ra để parse PDF phức tạp thành Markdown phục vụ riêng cho RAG.

Mục tiêu là biến file văn bản thô thành các block text có cấu trúc, gắn chặt với metadata_mapping.json.

Công cụ: Python, regex, json.

Thực thi:

Script đọc tên file (vd: 59_2020_QH14.docx), đối chiếu với metadata_mapping.json để lấy mã_văn_bản và tên_văn_bản.
...
Lưu ý xóa collection đã tồn tại để đảm bảo tính toàn vẹn, sạch sẽ dữ liệu.
